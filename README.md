...
- GitHub Actions 每 6 小时检查一次上游；通过测试的 `upstream/main` 变更会合并到本仓库 `main`，不会强制推送。
- 上游发布正式 Release 后，自动化从对应的不可变 tag 生成 `stable/<上游版本>`，重放 `patches/customizations` 中的本地定制，完成回归测试、依赖安装和安装包校验后才发布。
- fork 与上游共用 tag 命名空间，不能安全复用上游原 tag。缺失的补丁位会补 `0`，再追加 fork 修订号：上游 `2.19` 对应 `2.19.0.1001`，上游 `2.19.1` 对应 `2.19.1.1001`。初代版本 `2.18.1001` 保持不变。
- 合并冲突、补丁语义变化、测试失败或上游修改 GitHub Actions 文件时，流程会停止且自动创建 Issue，不会移动已有稳定分支或 tag。
- 新增本地定制时，需要同时以线性提交维护到 `patches/customizations`；该分支只保存定制补丁，不混入每次发布的版本提交。

面板首页只读取本仓库最新的正式 Release。上游 `main` 的日常变化不会直接提示线上升级，只有经过稳定分支构建并成功发布的安装包才会成为新版本。
